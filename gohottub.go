...
const DefaultTempTarget float64 = 100.0           //default; overwritten by ThingSpeak
...
func main() {
	...
	for { //evar
		...
		log("    DS18b20 temperature: " + strconv.FormatFloat(s.temperature, 'f', 2, 64))
		...
		heatRate := s.HeatRate()
		log("              Heat rate: " + strconv.FormatFloat(heatRate, 'f', 2, 64) + " (netagive = cooling)")
		...
		if s.isSafe {                                        // Only regulate temp/relays if in Safe mode!
			log("             Loop error: " + strconv.FormatFloat(loopError, 'f', 2, 64) + " (positive = below target)")
			if s.HeaterState() {
